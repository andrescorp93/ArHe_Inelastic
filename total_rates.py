...
for d in dirs:
    ...
    data = np.loadtxt(f'{d}/rate_const_airy_detailed.txt', skiprows=1).transpose()
...
np.savetxt(f'rate_const_s_airy_detailed.txt', out_data_s, fmt='%.6e', delimiter='\t', header=header_s, comments='')
...
np.savetxt(f'rate_const_p_airy_detailed.txt', out_data_p, fmt='%.6e', delimiter='\t', header=header_p, comments='')
