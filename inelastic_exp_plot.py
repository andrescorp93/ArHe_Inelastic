        ...
        for i in range(nt):
            for j in range(nt):
                if i != j:
                    ...
                    d[:,i,j] = 2*(als[:,i,j]+addr[:,i,j])
        for l in range(lmax):
            s[l] = np.dot(s0[l], np.dot(expm(1j*d[l]), s0[l]))
        for i in range(nt):
            for j in range(i+1, nt):
                    if any([np.abs(c) >= 1e-6 for c in np.abs(als[:,i,j])]):
                        plt.plot(js, np.abs(als[:,i,j]), 'o-', label='$|A_{2p_9 \\rightarrow 2p_8}|$')
                    if any([np.abs(c) >= 1e-6 for c in np.abs(addr[:,i,j])]):
                        plt.plot(js, np.abs(addr[:,i,j]), 'o-', label='$|B_{2p_9 \\rightarrow 2p_8}|$')
                    if any([np.abs(c) >= 1e-6 for c in np.abs(s[:,i,j])]):
                        plt.plot(js, np.abs(s[:,i,j]), 'o-', label='$|S_{2p_9 \\rightarrow 2p_8}|$')
        plt.xlabel('$L$')
        plt.legend()
        plt.savefig(f'images/Amps_exp_{int(np.round(e * 4.637 + np.real(hls[-1,0,0])))}.eps')
        plt.close()
    # print(sigmadiff[k])
