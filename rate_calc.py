    ...
    for i in range(n):
        for j in range(n):
            if i != j:
                ...
                coeffs[t,i,j] = trapezoid(maxwell(thetas[:,i], temperatures[t])*vels[:,i]*sigmas[:,j,i],thetas[:,i])
...
np.savetxt(f'{dir}/rate_const_exp.txt', tofile, fmt='%.6e', delimiter='\t', header=headerfinal, comments='')
# for i in range(n):
